...
'''Classes'''
# Camera object handles the scrolling of the screen
class Camera(object):
...
# HUD object shows player information during the game
class HUD(object):
    ...
    for y in range(len(map)):
        for x in range(len(map[y])):
            ...
            if tile == " ":
                ...
                coin = Item(name, pos[0], pos[1], tile_dim[0], tile_dim[1], YELLOW, rand.randint(10, 40), ["money", 1, 0])
                for Group in (Drawables, Collidables):
                    Group.add(coin)
            elif tile == "T":
                chest = Item(name, pos[0], pos[1], tile_dim[0], tile_dim[1], (0, 222, 240), rand.randint(50, 100),
                             ["money", 20, 0])
                for Group in (Drawables, Collidables):
                    Group.add(chest)
            elif tile == "B":
                bandage = Item(name, pos[0], pos[1], tile_dim[0], tile_dim[1], GREEN, 0, ["health", 1, 0])
                for Group in (Drawables, Collidables):
                    Group.add(bandage)
            elif tile == "M":
                medkit = Item(name, pos[0], pos[1], tile_dim[0], tile_dim[1], RED, 0, ["health", 999, 0])
                for Group in (Drawables, Collidables):
                    Group.add(medkit)
            elif tile == "R":
                rum = Item(name, pos[0], pos[1], tile_dim[0], tile_dim[1], (255, 184, 17), rand.randint(100, 200),
                           [["health", 3, 0], ["slow", 3, 10000]])
                for Group in (Drawables, Collidables):
                    Group.add(rum)
            elif tile == "H":
                horse = Item(name, pos[0], pos[1], tile_dim[0], tile_dim[1], (118, 78, 37), rand.randint(100, 200),
                             ["fast", 5, 20000])
                for Group in (Drawables, Collidables):
                    Group.add(horse)
            elif tile == "A":
                anchor = Obstacle(name, pos[0], pos[1], tile_dim[0], tile_dim[1], BLACK, 1, 200, 6000)
                for Group in (Drawables, Collidables):
                    Group.add(anchor)
            elif tile == "N":
                barrel = Obstacle(name, pos[0], pos[1], tile_dim[0], tile_dim[1], (128, 128, 128), 1, 200, 3000, BarrelSprite)
                for Group in (Drawables, Collidables):
                    Group.add(barrel)
            elif tile == "O":
                crate = Obstacle(name, pos[0], pos[1], tile_dim[0], tile_dim[1], (0x99, 0x33, 0x11), 1, 200, 2000)
                for Group in (Drawables, Collidables):
                    Group.add(crate)
            elif tile == "P":
                global playerSprite
...
if __name__ == "__main__":
    ...
    BarrelSprite = pygame.image.load("images\\Normal barrel.png")
    ...
    # The menu that handles the two buttons on the Game Over screen
    ...
    # Clickable pause button on the top right corner
    pauseButton = Button("pause", round(1880*xpos_f), 0, pauseButtonSurf, pauseButtonSurf, pauseButtonSurf)

    # List containing all GUI objects and menus to be adjusted to fit the new resolution
    ...
    while not close:
        ...
        if mode == "main":
            ...
            # Notifies user that the feature he clicked on is not yet available
            if tell:
        ...
        # Settings menu
            ...
            Settings.draw()
            # Bind left key
            # Countdown is drawn if binding procedure is active
            if action_bind == "left":
                label = subFontRes.render(str(int(bind_time/1000)+1)+"...", BLACK)[0]
            # Otherwise, the assigned key name is drawn
            else:
            ...
            pos[0] += 10 * xpos_f
            Window.blit(label, pos)
            # Note to user that there is no audio or difficulty settings
        ...
        # Gameplay mode
            ...
            if not loaded:
                # Sprite groups are cleared by re-instanced
                Drawables = pygame.sprite.LayeredUpdates()
                playerGroup = pygame.sprite.GroupSingle()
                Collidables = pygame.sprite.Group()
                Platforms = pygame.sprite.Group()
                # Course map and level data is read from the current level file
                Map, Metadata = loadLevel("levels\lvl"+str(level)+".ldat")
                # Course dimensions from metadata
                length = int(Metadata["length"])
                height = int(Metadata["height"])
                # Position of goal point in the course
                ...
                # Instances the Wave obstacle and adds to Drawables and Collidables
                ...
                # How many tiles the player has traversed
                tile_progress = 0
                # Game is now loaded
                loaded = True
            ...
            # Updates game when not paused
            if not pause:
                # If the player is not in combat
                if playerSprite.get_opponent() == None:
                    # Retrieves a list of all sprites that are colliding with the player
                    collide_list = playerSprite.update(inputs, Collidables, Platforms, playerGroup)
                    # Updates all collidable sprites
                    Collidables.update(collide_list, playerSprite)
                    # Sets the scroll speed depending on how the player is moving
                    GameCam.set_scroll(playerSprite)
                    # Applies the scrolling to all drawable sprites
                    ...
                    # Points added depending on number of tiles covered
                    ...
                    enemy = playerSprite.get_opponent()             # Enemy currently in battle
                    # Initialises combat game
                    if not combat_init:
                        # Fetches player and enemy health
                        player_hp = playerSprite.get_hp()
                        enemy_hp = enemy.get_hp()
                        # Fetches player and enemy attack strength
                        player_sp = playerSprite.get_sp()
                        enemy_sp = enemy.get_sp()
                        # Fetches player and enemy defence
                        player_ap = playerSprite.get_ap()
                        enemy_ap = enemy.get_ap()
                        attack_key = rand.randint(97, 122)          # Randomly chooses the first attack key
                        progress = 0                                # Position of pointer in combat bar
                        playerSprite.user_move(inputs, [enemy])     # Player collides into the enemy
                        combat_init = True                          # Combat game initialised
                    # Player attacks if attack key is pressed
                    if attack_key in inputs["key"]:
                        # Attack damaged calculated as player strength reduced by the proportion of the enemy's defence
                        attack = player_sp * (1 - enemy_ap)
                        progress += attack
                        inputs["key"].remove(attack_key)
                        # Attack key randomly chosen
                        if rand.random() <= 1/5:
                            attack_key = rand.randint(97, 122)
                    # Random chance of enemy attack (1/120 chance)
                    if rand.random() <= 1/120:
                        # Attack calculated similar to player's attack
                        attack = enemy_sp * (1 - player_ap)
                        progress -= attack
                    # Player loses if progress is at minus player health
                    if progress <= -player_hp:
                        enemy.player_lose(playerSprite)
                        playerSprite.set_opponent(None)
                        combat_init = False
                    # Player wins if progress is at enemy health
                ...
                # Game paused if escape key is pressed
                if pygame.K_ESCAPE in inputs["key"]:
                    pause = True
                    inputs["key"].remove(pygame.K_ESCAPE)
            # While game is paused
            elif pause:
                # Game unpaused when escape key pressed or pause button triggered
                if pygame.K_ESCAPE in inputs["key"] or pauseButton.get_trigger():
                    pause = False
                    if pygame.K_ESCAPE in inputs["key"]:
                        inputs["key"].remove(pygame.K_ESCAPE)

            # Player loses if health is 0 or less
            if playerSprite.get_hp() <= 0:
                mode = "lose"
            # Level passed if player collides with the level clear area
            ...
            # Draws pause overlay if game is paused
            if pause:
