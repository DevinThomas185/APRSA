...
window_name = "Wave Run"
avail_resmodes = ["1920x1080", "1920x1200", "1280x1024", "1366x768", "1280x800",
                  "1440x900", "1600x900", "1024x768", "1680x1050"]      # List of supported resmodes
screen_resmodes = pygame.display.list_modes()       # List of resmodes available to the monitor
# Omits any unsupported resolution modes from the list
i = 0
while i < len(avail_resmodes):
    ...
    if resmode_tuple not in screen_resmodes:
        ...
        i += 1
resolution = (pygame.display.Info().current_w, pygame.display.Info().current_h)     # Resolution set to current monitor resolution
ticks = 60                                                          # Maximum number of game updates per second
Window = pygame.display.set_mode(resolution, pygame.FULLSCREEN)     # Sets up game screen and screen Surface
pygame.display.set_caption(window_name)                             # Sets program name
Clock = pygame.time.Clock()                                         # Handles game update frequency
# Dimensions of each tile of each level
tile_dim = (40, 40)

# Scale factor variables for adjusting objects to fit the changed resolution
scale_f = 1.0
xpos_f = 1.0
ypos_f = 1.0
...
if resmode in avail_resmodes:
    default_settings["resolution"] = resmode        # Default resolution set to monitor resolution if supported
else:
    print("Error: Your native resolution is not supported")     # Error printed if monitor resolution not supported
